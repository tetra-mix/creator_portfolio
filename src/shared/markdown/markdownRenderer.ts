...
export function renderMarkdownToCanvas(
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    // Table (borderless): pipe-separated rows
      ...
      const rows: string[][] = rowStrs.map((r) =>
        r
          .split('|')
          .slice(1, -1)
          .map((c) => c.trim()),
  ...
  const flushLine = () => {
    ...
    for (const seg of lineSegments) {
      ...
      if (seg.linkUrl && linkRects) {
        const ascent = (metrics as TextMetrics).actualBoundingBoxAscent ?? lineHeight * 0.8;
        const descent = (metrics as TextMetrics).actualBoundingBoxDescent ?? lineHeight * 0.2;
