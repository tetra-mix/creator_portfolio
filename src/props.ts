...
function createPencil(): THREE.Object3D {
  ...
  const bandMat = new THREE.MeshStandardMaterial({ color: eraserColor, metalness: 0.2, roughness: 0.4 })
...
function createMug(): THREE.Object3D {
  ...
  const innerGeo = new THREE.CylinderGeometry(radiusTop - thickness, radiusBottom - thickness, height * 0.95, 24)
  innerGeo.scale(1, 1, 1)
  const innerMat = new THREE.MeshStandardMaterial({ color: 0xffffff, side: THREE.BackSide, roughness: 0.6 })
